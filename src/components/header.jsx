...
export default function Header({ wallet }) {
  const { account, isCorrectNetwork, connectWallet, disconnectWallet, switchToArcTestnet, isConnecting, error } = wallet;

  // Utility to format address to 0x1234...abcd
  const formatAddress = (addr) => `${addr.substring(0, 6)}...${addr.substring(addr.length - 4)}`;

  return (
    <header className="w-full bg-arc-card border-b border-arc-border px-6 py-3">
      ...
      {/* Previously silent: connectWallet()/switchToArcTestnet() errors were
          caught into wallet.error by useArcWallet but never shown anywhere.
          A failed connect (e.g. no wallet extension detected) looked
          identical to the button just not working. */}
      {error && (
        <div className="mt-2 text-xs font-mono text-arc-red bg-arc-red/10 border border-arc-red/30 rounded px-3 py-1.5">
          {error}
        </div>
      )}
    </header>
